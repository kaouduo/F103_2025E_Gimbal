#include "Algorithm.h"
#include "usart.h"
// #include "kalman_filter.h"
// #include "usbd_cdc_if.h"
// #include "usbd_core.h"
// #include "usbd_cdc.h"

//此文件是发送欧拉角的数值到上位机
#define MAX_BUFFER_SIZE 1024
uint8_t send_buf[MAX_BUFFER_SIZE];
uint16_t cnt = 0;
extern float roll,pitch,yaw;
extern float temp;
extern float roll_mahony, pitch_mahony, yaw_mahony;
extern float H723_Temperature;

// 时间测量变量
extern float mahony_update_time_us;
extern float mahony_compute_time_us;
extern float total_time_us;
/**
 * @brief 发送数据给上位机
 */
void vofa_start(void)   //在freertos 的默认任务里调用发送
{
	vofa_demo();		// 发送欧拉角数值
}

/**
 * @brief 修改通信工具，USART或者USB
 * @param buf       要发送的数据缓冲区指针
 * @param len       要发送的数据长度
 */
void vofa_transmit(uint8_t* buf, uint16_t len)
{
	// CDC_Transmit_HS((uint8_t *)buf, len);
	HAL_UART_Transmit(&huart1, (uint8_t *) buf, (uint16_t) len, 0xFF);

}

/**
 * @brief 将浮点数据拆分成单字节存入发送缓冲区
 * @param num       数据编号
 * @param data      要发送的浮点数据
 */
void vofa_send_data(uint8_t num, float data) 
{
	send_buf[cnt++] = byte0(data);
	send_buf[cnt++] = byte1(data);
	send_buf[cnt++] = byte2(data);
	send_buf[cnt++] = byte3(data);
}
/**
 * @brief 给数据包发送帧尾
 */
void vofa_sendframetail(void) 
{
	send_buf[cnt++] = 0x00;
	send_buf[cnt++] = 0x00;
	send_buf[cnt++] = 0x80;
	send_buf[cnt++] = 0x7f;
	
	/* 将数据和帧尾打包发送 */
	vofa_transmit((uint8_t *)send_buf, cnt);
	cnt = 0;// 每次发送完帧尾都需要清零
}
/**
 * @brief Demo示例：打包并发送所有需要监控的数据
 */
void vofa_demo(void)
{

	// Call the function to store the data in the buffer
	//===========================================================
	//ekf姿态解算的值
	vofa_send_data(0, pitch);
	vofa_send_data(1, roll);
	vofa_send_data(2, yaw);
	//==========================================================

	//====================================================
	//mahony解算的值
	vofa_send_data(3, pitch_mahony);
	vofa_send_data(4, roll_mahony);
	vofa_send_data(5, yaw_mahony);
	//========================================================

	vofa_send_data(6, temp); //陀螺仪加热温度
//	vofa_send_data(7, H723_Temperature);//h723内部温度

	//====================================================
	//时间测量数据 (us)
	vofa_send_data(7, mahony_update_time_us);     // Mahony_update 耗时
	vofa_send_data(8, mahony_compute_time_us);    // Mahony_computeAngles 耗时
	vofa_send_data(9, total_time_us);             // 总耗时
	//========================================================

	// Call the function to send the frame tail
	vofa_sendframetail();
}


